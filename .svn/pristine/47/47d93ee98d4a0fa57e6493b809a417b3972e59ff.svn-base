...
void set_bit(unsigned int* value, int position) {
    *value |= position;
}
	...
	for(i = 0; i < bitmap_size; i++){
		...
		for(j = 0; j < 8; j++){
			...
			if(bits2 == 0){
				...
				//set the bitmap
				//*bits |= j;
				set_bit((unsigned int *)(s_block->bitmap + i), nn);
				printf("aa\n");
				int block_pos = (i * 8) + j;
